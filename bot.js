...
client.on("message", message => {
  ...
  request.get("https://pinkie-api.glitch.me/api/kufur", (err, res, body) => { //Barış Demirci tarafından hazırlanmıştır xd
    ...
    if(contains) {
      message.delete();
      const cevaplar = [
        'bir daha duymamış olayım!',
        'Galiba küfür ediyor. Birileri bu kullanıcıyı durdursun!',
        'ağzını topla!',
        'sana kaç kere küfür etmemeni söylemeliyim?',
        'küfür etmeyi acilen bırakmalısın!',
        'Bu sunucuda küfür yasak!'
    ];
        var rand = cevaplar[Math.floor(Math.random() * cevaplar.length)];
        var cevap = rand.replace(message.author);
        message.reply(cevap);
      };
    }
  );
